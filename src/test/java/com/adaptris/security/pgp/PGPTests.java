...
abstract class PGPTests extends ExampleServiceCase {
  ...
  public void setUp() throws Exception {
    ...
    privateKey = new PGPSecretKey(PGPSignature.DEFAULT_CERTIFICATION, keyPair, ID, sha1Calc, null, null,
        new JcaPGPContentSignerBuilder(keyPair.getPublicKey().getAlgorithm(), HashAlgorithmTags.SHA1),
        new JcePBESecretKeyEncryptorBuilder(PGPEncryptedData.AES_256, sha1Calc).setProvider(PGPService.PROVIDER)
            .build(PASSPHRASE.toCharArray()));
    publicKey = privateKey.getPublicKey();
    /*
     * try (InputStream i = new FileInputStream("C:\\adaptris\\testkey.asc")) { PGPSecretKeyRingCollection pgpSec = new
     * PGPSecretKeyRingCollection(PGPUtil.getDecoderStream(i), new JcaKeyFingerprintCalculator()); Iterator keyRingIter =
     * pgpSec.getKeyRings(); while (keyRingIter.hasNext() && privateKey == null) { PGPSecretKeyRing keyRing =
     * (PGPSecretKeyRing)keyRingIter.next(); Iterator keyIter = keyRing.getSecretKeys(); while (keyIter.hasNext() && privateKey == null) {
     * PGPSecretKey key = (PGPSecretKey)keyIter.next(); if (key.isSigningKey()) { privateKey = key; } } } } try (InputStream i = new
     * FileInputStream("C:\\adaptris\\testkey.asc.pub")) { PGPPublicKeyRingCollection pgpPub = new
     * PGPPublicKeyRingCollection(PGPUtil.getDecoderStream(i), new JcaKeyFingerprintCalculator()); Iterator keyRingIter =
     * pgpPub.getKeyRings(); while (keyRingIter.hasNext() && publicKey == null) { PGPPublicKeyRing keyRing =
     * (PGPPublicKeyRing)keyRingIter.next(); Iterator keyIter = keyRing.getPublicKeys(); while (keyIter.hasNext() && publicKey == null) {
     * PGPPublicKey key = (PGPPublicKey)keyIter.next(); if (key.isEncryptionKey()) { publicKey = key; } } } }
     */
  }
   ...
   *
   * @param passphrase
   *          If true, the payload will be the private key passphrase.
   *
   * @return A new message.
   *
   * @throws Exception
  ...
  protected MultiPayloadAdaptrisMessage newMessage(boolean passphrase) throws Exception {
    ...
    return (MultiPayloadAdaptrisMessage) factory.newMessage(PAYLOAD_PLAINTEXT, passphrase ? PASSPHRASE : MESSAGE,
        factory.getDefaultCharEncoding());
  }
  ...
  protected MultiPayloadAdaptrisMessage newMessage(MultiPayloadAdaptrisMessage original) throws Exception {
    MultiPayloadAdaptrisMessage message = (MultiPayloadAdaptrisMessage) new MultiPayloadMessageFactory().newMessage(PAYLOAD_CIPHERTEXT,
        original.getPayload(PAYLOAD_CIPHERTEXT));
   ...
   * @param armor
   *          Whether to ASCII armor the key.
   *
   * @return The key.
   *
   * @throws Exception
   ...
   *
   * @param armor
   *          Whether the key will be ASCII armor encoded.
   *
   ...
   *
   * @param armor
   *          Whether the cipher text will be ASCII armor encoded.
   *
