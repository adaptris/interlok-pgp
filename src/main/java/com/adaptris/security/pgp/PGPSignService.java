...
/**
 * This service provides a way to sign messages via GPG/PGP. It requires a private key, the passphrase to unlock the key, and a message to
 * sign. Optionally it will ASCII armor encode the signature (default) and create a detached signature (default).
 *
 * <pre>{@code
 *    <pgp-sign>
...
public class PGPSignService extends PGPService {
   ...
   * @param out
   * @param out
   *          The generated signataure.
   * @throws PGPException
   *           Thrown if there's a problem with the key/passphrase.
   * @throws IOException
   *           Thrown if there's an IO issue.
   * @throws SignatureException
   *           Thrown if there's a problem creating the signature.
   */
  private static void sign(InputStream in, InputStream key, char[] passwd, int digest, OutputStream out)
    ...
    PGPSignatureGenerator sGen = new PGPSignatureGenerator(
        new JcaPGPContentSignerBuilder(pgpSec.getPublicKey().getAlgorithm(), digest).setProvider(PROVIDER));
   ...
   *          The digest algorithm to use.
   * @param armor
   *          Whether to armor encode the signature.
   * @param out
   *          The generated signataure.
   * @throws PGPException
   *           Thrown if there's a problem with the key/passphrase.
   ...
   *           Thrown if there's an IO issue.
   */
  private static void sign(InputStream in, InputStream key, char[] passwd, int digest, boolean armor, OutputStream out)
