...
/**
 * This service provides a way to verify GPG/PGP signed messages. It requires the public key of whom signed the message, the signed message,
 * and (if the signature is detached) the signature. It will will also optionally return the original/unsigned message (especially useful if
...
public class PGPVerifyService extends PGPService {
   ...
   *          The public key.
   * @param out
   *          The original, unsigned message.
   * @throws Exception
   *           Thrown if there's a problem verifying the signature.
   */
  ...
  private static void verify(InputStream inMessage, InputStream inSignature, InputStream key, ByteArrayOutputStream out) throws Exception {
    ...
    PGPPublicKeyRingCollection pgpPubRingCollection = new PGPPublicKeyRingCollection(getDecoderStream(key),
