...
abstract class PGPService extends ServiceImp {
  ...
  /*
   * These following three methods came about because the BouncyCastle GPG API likes to work on streams (ideal for encrypting/signing files)
   * but an Interlok user may select a data input/output parameter that is a string - not a big deal as strings can easily become streams,
   * and if the cipher is armor encoded then using strings is probably the easy thing to do, but that may not always be the case. So here we
   * do our best to get everything as a stream (except the private key passphrase, which should always be a string; if it's raw binary data
  ...
  protected <T> void insertStream(AdaptrisMessage message, DataOutputParameter<T> parameter, ByteArrayOutputStream value) throws Exception {
    if (parameter instanceof StringPayloadDataOutputParameter) {
      parameter.insert((T) value.toString(getEncoding(message)), message);
      ...
      parameter.insert((T) new InputStreamWithEncoding(new ByteArrayInputStream(value.toByteArray()), null), message);
    } else if (parameter instanceof ByteArrayPayloadDataOutputParameter) {
      parameter.insert((T) value.toByteArray(), message);
    }
  }
   ...
   * <p>
   * If the initial bytes of the underlying stream are binary PGP encodings, then the stream will be returned directly, otherwise an
   * {@link ArmoredInputStream} is used to wrap the provided stream and remove ASCII-Armored encoding.
   * </p>
   *
   * @param in
   *          the stream to be checked and possibly wrapped.
   * @return a stream that will return PGP binary encoded data.
   * @throws IOException
   *           if an error occurs reading the stream, or initialising the {@link ArmoredInputStream}.
   */
